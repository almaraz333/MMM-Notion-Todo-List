...
Module.register("MMM-Notion-Todo-List", {
  ...
  getData: function() {
    // Call the function to fetch data from Notion
  ...
  fetchNotionData: async function() {
    ...
    const blocks = await this.retrieveBlockChildren(notion, pageId); // Fetch blocks from Notion
    const tasks = this.getTasksFromBlocks(blocks); // Process and extract tasks
    return tasks;
  },
  ...
  getTasksFromBlocks: function(blocks) {
    // Extract plain text from block items that are to-do items
    const tasks = blocks
      .filter((block) => block.type === "to_do") // Filter only to-do items
      .map((block) => {
        return {
          text: getPlainTextFromRichText(block.to_do.rich_text),
          checked: block.to_do.checked, // Whether the task is checked off
        };
      });
    return tasks;
  },
  ...
  getDom: function() {
    ...
    this.tasks.forEach((task) => {
      ...
      listItem.textContent = task.checked
        ? `☑️ ${task.text}` // Mark completed tasks
        : `⬜ ${task.text}`; // Mark uncompleted tasks
